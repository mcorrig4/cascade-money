...
test('both real splice modes retain audio across alternating mono/stereo parts', () => {
  ...
  const required = Object.keys(lock).map(scene => `scene-${scene.padStart(2, '0')}.wav`);
  ...
  try {
    ...
    for (const name of ['splice-draft.sh', 'check-cues.mjs']) copyFileSync(fileURLToPath(new URL(name, import.meta.url)), join(root, 'scripts', name));
    ...
    for (let i = 1; i <= 12; i++) for (const dir of ['parts', 'parts-final'])
      copyFileSync(join(root, `${i % 2 ? 'mono' : 'stereo'}.mp4`), join(root, 'out', dir, `scene-${String(i).padStart(2, '0')}.mp4`));
