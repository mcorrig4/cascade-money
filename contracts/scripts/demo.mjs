...
async function main() {
  ...
  if (values.help) {
    console.log('node scripts/demo.mjs [--amount 10|100] [--seed text] [--vault address] [--broadcast] [--live] [--rpc URL] [--zero-income-catch-up] [--state-dir path]');
    console.log('Default is an offline plan targeting the local Arc fork. --broadcast executes locally; live requires --live and ARC_DEPLOYER_KEY.');
    return;
  }
  ...
  const seed = values.seed ?? process.env.CASCADE_DEMO_SEED ?? (values.live ? randomBytes(32).toString('hex') : LOCAL_SEED);
  ...
  const manifestPath = resolve(ROOT, values.live ? `deployments/${config.chainId}.json` : '.local/default-vault.json');
  const address = getAddress(values.vault ?? JSON.parse(readFileSync(manifestPath)).vault);
  const rpc = values.rpc ?? (values.live ? config.rpcUrl : process.env.CASCADE_FORK_RPC ?? FORK_RPC);
  const provider = new JsonRpcProvider(rpc);
  try {
    if (!values.live) await assertFork(provider, rpc);
    ...
    const directory = resolve(values['state-dir'] ?? resolve(ROOT, '.demo-runs', keccak256(toUtf8Bytes(address + seed + (values.live ? 'live' : 'fork')))));
    const identity = { mode: values.live ? "live" : "fork", chainId: config.chainId, vault: address, usdc: getAddress(config.usdc),
