...
Shot 1 uses a 760 m-high oblique campus camera with a gentle six-degree orbit and southwest sun at 32° elevation. Shot 10 approaches the cube from the southeast at 2.4 m above the site, with a 3° dusk sun, then returns to the globe. Both use the same sun vector for the Earth texture and directional model lighting, plus soft ambient. Run `pnpm --dir app check:browser --static` outside restricted sandboxes to capture `app/artifacts/apple-park-1920x1080.png` and `app/artifacts/fifth-avenue-1920x1080.png`. The check requires bundled GLBs to decode, accepts fallbacks when files are absent, and checks model release after the pullback.
