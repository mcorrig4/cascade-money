...
export function GlobeScene({ engine }: { engine: PlaybackEngine }) {
  ...
  useEffect(() => {
    ...
    globe.backgroundColor('#00000000').globeImageUrl(earthUrl)
      .showAtmosphere(true).atmosphereColor('#315d72').atmosphereAltitude(0.1)
    ...
    camera.near = 0.0005; camera.updateProjectionMatrix();
    globe.controls().minDistance = globe.getGlobeRadius() + 0.002;
    ...
    const frame = (now: number) => {
      ...
      cursor = state.cursor;
      pool.tick(time);
      if (now - lastColor > 33 || incoming.length) {
    ...
    if (new URLSearchParams(location.search).has('inspect')) {
      window.__cascade = { engine, globe, pool, ageArcs: (milliseconds: number) => { time += milliseconds; pool.tick(time); globe.arcColor(colorAccessor); },
