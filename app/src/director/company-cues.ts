...
export function companyCues(state:PlaybackState) {
  ...
  const orders=(shot?.orderCues??[]).map(cue=>({company:cue.word==='Samsung'?'Samsung Display':cue.word,atMs:state.cues[cue.word]??cue.at*1000*scale}));
  return [...orders,...COMPANY_BEATS.filter(b=>b.shot===state.shot).map(b=>({company:b.company,atMs:b.at*1000*scale})),...state.companyCues];
}
