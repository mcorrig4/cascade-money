...
export function SceneLabels({ shot, elapsed }: { shot: number | null; elapsed: number }) {
  ...
  return <>
    ...
    {cue && <div className="scene-narration" aria-label="Scene narration" data-cue={cue.id} style={{ opacity: cue.opacity, transform: `translateY(${cue.offset}px)` }}><p>{cue.text}</p></div>}
  </>;
}
