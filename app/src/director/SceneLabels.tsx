import { recordingElementVisible } from './recording.ts';
...
export function SceneLabels({ shot, elapsed, recording=false, cues={}, orders=[], payments=[] }: { recording?:boolean; payments?:Event[]; orders?:Event[]; shot: number | null; elapsed: number; cues?:Record<string,number> }) {
  ...
  return <>
    ...
    {cue && recordingElementVisible(recording,'scene-narration') && <div className="scene-narration" aria-label="Scene narration" data-cue={cue.id} style={{ opacity: cue.opacity, transform: `translateY(${cue.offset/10}vh)` }}><div>{cue.id==='derivatives'&&<span className="scene-cue-context">THE DERIVATIVES MARKET</span>}<p>{cue.text}</p></div></div>}
  </>;
}
