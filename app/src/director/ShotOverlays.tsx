...
import { recordingElementVisible, shotOverlayVisible } from './recording.ts';
...
export function ShotOverlays({ engine, state, onVerify }: { engine: PlaybackEngine; state: PlaybackState; onVerify?: () => void }) {
 const scene=SHOTS.find(s=>s.id===state.shot);if(!scene||!shotOverlayVisible(state,scene,scene.overlay)||['none','title'].includes(scene.overlay))return null;
 ...
 return <div className={`shot-overlay scene-overlay-${kind}`} data-testid={`overlay-${state.shot}`}>
  {recordingElementVisible(state.recording,'globe-dimmer')&&<div className="globe-dimmer"/>}
  ...
  {kind==='question'&&<section className="overlay-card question-card" aria-label="A future payment moves today" style={motion('question-card',0)}><h2>What if that future payment<br/>could move today?</h2><DatedDollar days={shown('coin-date',5.2)?datedUnit(115,state.day).days:null} isoDate={shown('coin-date',5.2)?datedUnit(115,state.day).isoDate:undefined} size={140}/></section>}
