...
export function ShotOverlays({ engine, state, onVerify }: { engine: PlaybackEngine; state: PlaybackState; onVerify?: () => void }) {
 ...
 return <div className={`shot-overlay scene-overlay-${kind}`} data-testid={`overlay-${state.shot}`}>
  ...
  {kind==='totals'&&<section className="overlay-card cascade-totals" aria-label="The cascade"><div className="cascade-stat" style={motion('committed-counter',0)}><strong>{dollars(DEFAULT_CASCADE.committed,true)}</strong><span>committed</span></div><div className="cascade-stat" style={motion('settled-counter',2)}><strong>{dollars(DEFAULT_CASCADE.settled,true)}</strong><span>obligations settled</span></div><div className="cascade-stat" style={motion('companies-counter',4.8)}><strong>{DEFAULT_CASCADE.companies}</strong><span>companies paid</span></div><h2 style={motion('tagline',10)}>That’s the cascade.</h2></section>}
