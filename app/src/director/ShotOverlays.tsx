...
export function ShotOverlays({ engine, state, onVerify }: { engine: PlaybackEngine; state: PlaybackState; onVerify?: () => void }) {
 ...
 const coinNames=['coin','coin-date','coin-fungibility','coin-claim','coin-extend','coin-yield','coin-return'];
 const coinIndex=Math.max(0,COIN_BEATS.findLastIndex((b,i)=>shown(coinNames[i],b.at)));
 const coinBeat=COIN_BEATS[coinIndex],progress=Math.min(1,Math.max(0,(tMs-at('coin-extend',15.2))/Math.max(1,at('coin-return',25.2)-at('coin-extend',15.2)))),date=Math.round(30+60*progress);
 ...
 return <div className={`shot-overlay scene-overlay-${kind}`} data-testid={`overlay-${state.shot}`}>
  ...
  {kind==='totals'&&<section className="overlay-card cascade-totals" aria-label="The straight-line cascade"><div className="cascade-stat" style={motion('committed-counter',0)}><strong>{dollars(DEFAULT_CASCADE.committed,true)}</strong><span>committed</span></div><div className="cascade-stat" style={motion('settled-counter',2)}><strong>{dollars(DEFAULT_CASCADE.settled,true)}</strong><span>obligations settled</span></div><div className="cascade-stat" style={motion('companies-counter',4.8)}><strong>{DEFAULT_CASCADE.companies}</strong><span>companies paid</span></div><h2 style={motion('tagline',10)}>That’s the cascade.</h2></section>}
  {kind==='coin'&&<section className={`overlay-card coin-layout coin-${coinBeat.key}`} aria-label="Dated coin" data-beat={coinBeat.key}>
   <div className="coin-copy" key={coinBeat.key} style={motion(coinNames[coinIndex],coinBeat.at)}><span className="eyebrow">A DOLLAR WITH A DATE</span><h2>{coinBeat.title}</h2><p>{coinBeat.text}</p>
    {shown('coin-extend',15.2)&&!shown('coin-return',25.2)&&<div className="coin-extension"><div className="date-interval"><span>DAY 30</span><span className="interval-line"/><span>DAY 90</span></div><div className="yield-track"><div style={{width:`${progress*100}%`}}/></div><div className="meter-label"><span>{Math.round(60*progress)} added days</span><span>Yield for this interval</span></div></div>}
   </div><div className="coin-stage" style={motion('coin',0)}><div className="dated-coin"><span>CASCADE</span><strong>$1</strong>{shown('coin-date',4.8)&&<><div className="coin-date">{displayDate(date)}</div><small>DAY {date}</small></>}</div>
  ...
  {kind==='backing'&&<section className="overlay-card backing-card" aria-label="Vault backing" style={motion('backing-card',0)}><span className="eyebrow">UNDERNEATH IT</span><h2>A vault on Arc.</h2><div className="asset-composition"><b>USDC <small>backing</small></b><span>→</span><b>Arc vault</b><span>←</span><b>USYC <small>designed yield reserve</small></b></div><p>One shared account of who owns what.<br/>And when it becomes cash.</p><button className="onchain-trigger" onClick={onVerify}>Verify on Arc</button></section>}
