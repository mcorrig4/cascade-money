...
export function ShotOverlays({ engine, state }: { engine: PlaybackEngine; state: PlaybackState }) {
  ...
  const extension = engine.index.extensions.findLast(e => e.seq <= seq);
  const progress = Math.min(1, Math.max(0, (state.shotElapsed - 25) / 28));
  ...
  return <div className={`shot-overlay ${cube ? 'cube-overlay' : ''}`} data-testid={`overlay-${shot}`}>
    ...
    {shot === 6 && <section className="overlay-card coin-layout" aria-label="Dated coin">
      <div><span className="eyebrow">A DOLLAR WITH A DATE</span><h2>Time moves<br />forward.</h2><p>One dollar. Redeemable on its date.<br />An earlier dollar pays a later bill at face value.</p><div className="date-interval"><span>DAY {from}</span><span className="interval-line" /><span>DAY {to}</span></div><div className="yield-track"><div style={{ width: `${progress * 100}%` }} /></div><div className="meter-label"><span>{Math.round((to - from) * progress)} added days</span><span>Accrued yield {accrued === null ? '—' : dollars(accrued)}</span></div><p className="fine-print">The added interval earns the vault’s yield.</p></div>
      <div className="dated-coin"><span>CASCADE · DATED DOLLAR</span><strong>$1</strong><div key={date} className="coin-date">{displayDate(date)}</div><small>DAY {date}</small></div>
    </section>}
    ...
    {shot === 10 && (cube ? <section className="cube-caption"><span className="eyebrow">FIFTH AVENUE, NEW YORK</span><h2>Money. And time.</h2></section> : <section className="overlay-card reframe-card"><span className="eyebrow">THE DERIVATIVES MARKET</span><h2>$846<span>trillion</span></h2><p>Time becomes a property of money.</p></section>)}
    {shot === 11 && <section className="overlay-card architecture-card" aria-label="Architecture"><span className="eyebrow">DATED DOLLARS ON ARC</span><h2>One vault. Composable time.</h2><img src={`${import.meta.env.BASE_URL}architecture.svg`} alt="USDC enters the Arc vault. Issue dated dollars, pay, extend with yield entitlements, and claim or withdraw. A separate discount window exchanges dated dollars for spot." /><a href={`${import.meta.env.BASE_URL}architecture.svg`} download>Download architecture SVG</a></section>}
    {shot === 12 && <section className="overlay-card close-card" aria-label="Cascade close card"><span className="eyebrow">DATED DOLLARS ON ARC</span><h2>cascade<span>.</span></h2><p>Money that pays bills<br />before it becomes cash.</p><nav aria-label="Cascade links"><a href="/architecture">Architecture</a><a href={appUrl}>Open the app <svg width="16" height="16" viewBox="0 0 16 16" aria-hidden="true"><path d="M3 13 13 3M3 3h10v10" fill="none" stroke="currentColor" strokeWidth="1.5" /></svg></a><a href="https://github.com/mcorrig4/cascade" target="_blank" rel="noreferrer">Repository <svg width="16" height="16" viewBox="0 0 16 16" aria-hidden="true"><path d="M3 13 13 3M3 3h10v10" fill="none" stroke="currentColor" strokeWidth="1.5" /></svg></a><a href="https://testnet.arcscan.app/address/0x57838a35f05a43ad519204d7a6ce63f52d7c1987#code" target="_blank" rel="noreferrer">Arc contract <svg width="16" height="16" viewBox="0 0 16 16" aria-hidden="true"><path d="M3 13 13 3M3 3h10v10" fill="none" stroke="currentColor" strokeWidth="1.5" /></svg></a></nav><div className="close-footer"><span>{import.meta.env.VITE_TEAM || 'Cascade team'}</span><span>Earth imagery: NASA</span></div></section>}
  </div>;
}
