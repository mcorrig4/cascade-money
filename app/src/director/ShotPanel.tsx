...
import { SHOTS, playShot, playFilm, nextShot, shotAvailable } from './shots.ts';
...
export function ShotPanel({ engine, state, onClose }: { engine: PlaybackEngine; state: PlaybackState; onClose: () => void }) {
  return <section className="director" aria-label="Shot director">
    ...
    <button className="record-button" onClick={()=>{playFilm(engine);engine.update({recording:true});onClose();}}>Play full film · 3:56</button>
