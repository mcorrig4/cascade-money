...
import { setRecordingMode } from './recording.ts';
...
export function ShotPanel({ engine, state, onClose }: { engine: PlaybackEngine; state: PlaybackState; onClose: () => void }) {
  return <section className="director" aria-label="Shot director">
    ...
    <button className="record-button" onClick={()=>{void engine.ready().then(()=>{playFilm(engine);setRecordingMode(engine,true);onClose();});}}>Play full film · {filmDuration()}</button>
    <button className="record-button" onClick={() => { void engine.ready().then(()=>{setRecordingMode(engine,true);onClose();}); }}><Icon name="expand" />Recording mode</button>
