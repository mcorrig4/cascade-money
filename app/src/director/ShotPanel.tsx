...
export function ShotPanel({ engine, state, onClose }: { engine: PlaybackEngine; state: PlaybackState; onClose: () => void }) {
  return <section className="director" aria-label="Shot director">
    ...
    <div className="director-controls"><button onClick={() => playShot(engine, Math.max(1, (state.shot ?? 2) - 1))}><Icon name="left" />Previous</button><button onClick={() => playShot(engine, state.shot ?? 1)}><Icon name="replay" />Restart</button><button onClick={() => playShot(engine, Math.min(5, (state.shot ?? 0) + 1))}>Next<Icon name="right" /></button></div>
