...
function LoadedApp({ index }: { index: EventIndex }) {
  ...
  return <main className={`app ${state.recording ? 'recording' : ''} ${ledgerOpen ? 'ledger-open' : ''} ${state.shot ? 'director-active' : ''} ${SHOTS.find(s=>s.id===state.shot)?.overlay!=='none'&&state.shot?'overlay-active':''} ${[1,2,13,10,19,12].includes(state.shot??0)?'scene-clean':''} ${state.shot===12?'ending-wordmark':''}`}>
    ...
    <DayLedger index={index} day={state.day} cursor={state.cursor} running={state.playing || state.shotRunning} rate={speedRate(state.speed)} waiting={state.paymentPresentation==='waiting'} presentation={state.paymentAmount!==null&&engine.storyEvents!==null?{events:engine.storyEvents,amount:state.paymentAmount,date:state.paymentMaturity}:undefined} />
