...
// Coin-v4 geometry stays frozen (Liam, 2026-09-13). W1 restores the original
// green .dated-coin palette from styles.css at b6826bc, without adding its third ring.
// The condensed date uses the bundled Coin Date Condensed font; callers supply its CSS.
const RING_STROKE = 7;
const PLUS_ARM = 34;
const PLUS_THICK = 20;
...
export function DatedDollar({ days = null, isoDate, size = 96 }: DatedDollarProps) {
  ...
  return h('svg', { xmlns: 'http://www.w3.org/2000/svg', viewBox: `18 18 ${width} 170`,
    ...
    h('defs', null, h('radialGradient', { id: gradient, cx: '38%', cy: '32%', r: '75%' },
      h('stop', { offset: '0%', stopColor: '#214b43' }), h('stop', { offset: '65%', stopColor: '#0a2624' }), h('stop', { offset: '100%', stopColor: '#071919' }))),
    h('g', { fontFamily: "Inter, 'DejaVu Sans', Helvetica, Arial, sans-serif" },
      ...
      h('text', { x: 100, y: 111, textAnchor: 'middle', fontSize: 34, fontWeight: 700, fontStyle: 'italic', letterSpacing: 1, fill: '#9aedd0' }, 'USD'),
      h('g', { transform: `translate(${PLUS_CENTER[0]} ${PLUS_CENTER[1]})`, fill: '#9aedd0' },
      ...
      h('text', { x: NUM_X, y: NUM_BASELINE, fontSize: NUMBER_SIZE, fontWeight: 700, letterSpacing: -2, fill: '#9aedd0', style: { fontVariantNumeric: 'tabular-nums' } }, shown),
