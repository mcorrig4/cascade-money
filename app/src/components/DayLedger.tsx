...
function LedgerRow({ event, index, compact, inspect, waiting, amount, date }: { event: Event; index: EventIndex; compact:boolean; inspect:()=>void; waiting:boolean; amount?:bigint; date?:number|null }) {
  ...
  return <article tabIndex={0} onClick={inspect} onFocus={inspect} className={`ledger-row ${event.type}`} data-seq={event.seq}>
    {compact ? <div className="ledger-log-line"><time dateTime={event.date} title={`Event ${event.seq}`}>{event.date.slice(5)}</time><span className="log-parties" title={`${name(from)} → ${name(to)}`}>{name(from)} → {name(to)}</span><strong>{dollars(amount??(event.amount || invoice?.amount || 0n),true)}</strong><span className="log-unit">{unit}</span></div> : <><div className="ledger-row-top"><span className="operation-dot" /><span>{waiting&&['issue','pay'].includes(event.type)?'Payment waiting':labels[event.type] ?? event.type.replaceAll('_', ' ')}</span><span className="sequence">{String(event.seq).padStart(3, '0')}</span></div>
    ...
    <div className="ledger-bottom"><strong>{dollars(amount??event.amount, true)}</strong><span>{invoice?.item && !detail ? invoice.item : event.type === 'issue' && typeof event.data.mint_date === 'number' ? `Dated ${displayDate(event.data.mint_date, true)}` : ''}</span></div>
