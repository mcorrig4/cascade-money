...
function VolumeChart({ index, position, growing }: { index: EventIndex; position: number; growing:boolean }) {
  ...
  return <div className="volume-chart"><span className="chart-ceiling">{dollars(maximum, true)}</span>
    <svg viewBox="0 0 1095 76" preserveAspectRatio="none" role="img" aria-label="Daily dollar volume of new purchases and settled invoices">
      ...
      <defs><clipPath id="elapsed-volume"><rect width={growing?Math.min(1095,position*3):1095} height="76"/></clipPath></defs><g clipPath="url(#elapsed-volume)"><path d={purchases} fill="none" stroke="#7e97a5" strokeWidth="2" vectorEffect="non-scaling-stroke" />
      <path d={settled} fill="none" stroke="#69e6c0" strokeWidth="2" vectorEffect="non-scaling-stroke" />
      </g><path d={`M${Math.min(1093, position * 3 + 1)} 0V76`} stroke="#f1f4e7" strokeWidth="1" vectorEffect="non-scaling-stroke" />
    </svg></div>;
}
export function Timeline({ engine, state }: { engine: PlaybackEngine; state: PlaybackState }) {
  ...
  return <footer className="bottom-panel">
    <div className="timeline-region">
      ...
      <div className="chart-track"><VolumeChart index={engine.index} position={state.position} growing={state.shot===5} /><input className="scrubber" type="range" min="0" max="364" step="1" value={state.day} onChange={e => engine.seek(Number(e.target.value))} aria-label="Timeline day" aria-valuetext={displayDate(state.day)} /></div>
